<header class="navbar navbar-fixed-top">
        <div class="navbar-inner">
            <div class="container">
                <a class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                    <span class="icon-bar"></span>
                </a>
                <a id="logo" class="pull-left" href="?view=main"></a>
                <div class="nav-collapse collapse pull-right">
                    <ul class="nav">
                        <li class="active"><a href="?view=main">Home</a></li>
                        <li><a href="?view=about">About Us</a></li>
                        <li><a href="?view=services">Services</a></li>
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">Forms <i class="icon-angle-down"></i></a>
                            <ul class="dropdown-menu">
                                <li><a href="?view=cust_care">Cutodial Care</a></li>
                                <li><a href="?view=fun_services">Funeral Service</a></li>
                                <li><a href="?view=burial">Burial</a></li>
                                <li><a href="?view=cremation">Cremation</a></li>
                                <li class="divider"></li>
                            </ul>
                        </li>
                        <li><a href="?view=contact">Contact</a></li>
                        <li class="login">
                            <a data-toggle="modal" href="#loginForm"><i class="icon-lock"></i></a>
                        </li>
                    </ul>        
                </div><!--/.nav-collapse -->
            </div>
        </div>
    </header>